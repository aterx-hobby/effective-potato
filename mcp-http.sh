...
# Load optional local environment config for the server process.
...
if [ -f "local/.env" ]; then
	...
	echo "Warning: local/.env not found (skipping)" >&2
fi

MODE=${POTATO_TOOLKIT:-}

if [ "${MODE}" = "review" ] || [ "${MODE}" = "review-only" ] || [ "${MODE}" = "review_only" ]; then
	echo "Starting HTTP server in review-only mode (no build)" >&2
else
...
# Control bind and route with:
#   POTATO_HOST (default 127.0.0.1)
#   POTATO_PORT (default 8000)
#   POTATO_MCP_PATH (default /mcp)
